...
public final class ContextMenu extends UIElement implements ContextMenuItemable {
    ...
    public static final int HEIGHT_PADDING = 5;
	...
	//TODO
	GlyphLayout glyphLayout = new GlyphLayout(font, e.getLabel());
	...
	for (ContextMenuItemable item : items) {
	    if (item instanceof ContextMenu) {
		ContextMenu conmen = (ContextMenu) item;
		conmen.setActivationListener(activationListener);
	    }
	}
    }
...
//	}
	...
	for (int i = 0; i < temporary.size(); i++) {
	    ...
	    for (ContextMenuItemable get : actions.get(i)) {
		contextMenu.add(get);
	    }
